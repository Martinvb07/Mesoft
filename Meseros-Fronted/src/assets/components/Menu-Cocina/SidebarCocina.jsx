...
import {
  HiOutlineFire, HiOutlineArrowRightOnRectangle, HiBars3, HiXMark,
  ...
  try {
    for (const k of ['currentUser', 'usuario', 'user', 'auth_user']) {
      ...
      if (raw) {
        ...
        const r = u?.rol || u?.role || u?.usuario?.rol || 'cocinero';
        const n = u?.nombre || u?.name || u?.usuario?.nombre || '';
        if (n || r) return { rol: r, nombre: n || 'Cocina' };
      }
    }
  } catch {}
  return { rol: 'cocinero', nombre: 'Cocina' };
}

const LINKS = [
  { to: '/cocinero/cocina', label: 'Cocina', icon: HiOutlineFire },
];
...
function ActiveBar() {
  return (
    <motion.span
      layoutId="cocina-sidebar-active-bar"
...
function Logo({ collapsed }) {
  return (
    <Link to="/cocinero/cocina" className="flex items-center gap-2.5 text-slate-900 no-underline">
      <motion.img
...
const SidebarCocina = ({ collapsed = false, onToggleCollapse = () => {} }) => {
          ...
          {!collapsed && (
            <div className="mb-1 flex items-center gap-2.5 rounded-xl px-2 py-1.5">
              ...
              <div className="min-w-0">
                ...
                <p className="m-0 truncate text-xs capitalize text-slate-400">{rol || 'cocinero'}</p>
              </div>
            </div>
          )}
          <button
